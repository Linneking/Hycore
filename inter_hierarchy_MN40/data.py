...
class ModelNet40(Dataset):
    ...
    def __getitem__(self, item):
        ...
        return pointcloud, label, item       # ← 新增: 返回全局样本索引

    def __len__(self):
        return self.data.shape[0]

    def get_class_counts(self):
        """返回每个类别的样本数，用于 class-balanced sampling"""
...
if __name__ == '__main__':
    ...
    for batch_idx, (data, label, idx) in enumerate(train_loader):
        print(f"batch_idx: {batch_idx}  | data shape: {data.shape} | label shape: {label.shape} | idx: {idx}")
        break
    ...
    print(f"class_counts: {train_set.get_class_counts()}")