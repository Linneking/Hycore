...
def parse_args():
    ...
    parser.add_argument('--model', default='Hype_PointNet', help='model name [default: pointnet_cls]')
...
def main():
    ...
    net = Hype_pointMLP() #models.__dict__[args.model]()
    criterion = cal_loss
    ...
    optimizer = geoopt.optim.RiemannianSGD( net.parameters() , lr= args.learning_rate, momentum=0.9, weight_decay=args.weight_decay)	#lr=0.1, momentum=args.momentum, weight_decay=1e-4)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        time_cost,std_loss,y_loss,h_loss,pd,nd,pn,posn,train_out = train(net, train_loader, optimizer, criterion, criterion2, device)  # {"loss", "acc", "acc_avg", "time"}
        ...
        if epoch%50==0:
            printf('Time: %.2f Epoch(%d/%s) Learning Rate %s:' % (time_cost,epoch + 1, args.epoch, optimizer.param_groups[0]['lr']))
...
def train(net, trainloader, optimizer, criterion, criterion2, device):
    ...
    for batch_idx, (data, label) in enumerate(trainloader):
        ...
        mar_par, data, n_points = get_children_np(data, kmin = 800, kmax = 1024)       #800-1024
        mar, pos_data, _ = get_children_np(data, starting = n_points, kmin = 200, kmax = 600)  #50-600

        #mar_par, data, n_points = get_children_adv(data, kmin = 600, kmax = 1024)       #800-1024
        #mar, pos_data, _ = get_children_adv(data, starting = n_points, kmin = 100, kmax = 300)  #50-600

        #mar, pos_data = get_children(data,)
    ...
    return time_cost, std_loss,t_loss,h_loss,pd,nd,pn,posn,{
